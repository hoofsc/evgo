...
open class YTSwiftyPlayer: WKWebView {
    ...
    public init(frame: CGRect = .zero, playerVars: [String: AnyObject]) {
        config = YTSwiftyPlayer.defaultConfiguration
        ...
        callbackHandlers.forEach {
            ...
            userContentController.add(leakAvoider, name: $0.rawValue)
        }
    ...
    public func cleanUp() {
        clearVideo()
        callbackHandlers.forEach {
            config.userContentController.removeScriptMessageHandler(forName: $0.rawValue)
        }
